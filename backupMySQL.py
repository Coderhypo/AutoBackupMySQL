...
def backup():
    ...
    print u"INFO: 开始执行数据库备份，时间:" + str(today)

    message = check()
    if message:
        print message + '\n'
        sendmail.send(u"[%s]%s数据库备份失败！" % (today, BACKUP_CONFIG['DATABASE_NAME']))
    ...
    try:
        print u"INFO: " + MYSQLDUMP
        os.system(MYSQLDUMP)
        print u'INFO: 成功导出数据库'
    except:
        print u'ERROR: 导出数据库失败！\n'
        sendmail.send(u"[%s]%s数据库备份失败！" % (today, BACKUP_CONFIG['DATABASE_NAME']))
        print u"\nINFO :执行结束， 时间:" + str(today)
        return 
    ...
    try:
        ...
        today_file_size = 0
    print u'INFO: 今日备份文件大小: ' + str(today_file_size)
    try:
        yesterday_file_size = os.path.getsize('%s%s.gz' % (BACKUP_CONFIG['BACKUP_DIR'], str(yesterday)))
    except:
        yesterday_file_size = 0
    print u'INFO: 查询昨日备份文件大小: ' + str(yesterday_file_size)

    print '\n'
    sendmail.send(u"[%s]%s数据库备份成功！" % (today, BACKUP_CONFIG['DATABASE_NAME']),
